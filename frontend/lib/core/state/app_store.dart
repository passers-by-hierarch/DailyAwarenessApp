...
class AppStore extends ChangeNotifier {
  ...
  void addAgenda(AgendaItem agenda) {
    ...
    final finalAgenda = agenda.id.isEmpty ? agenda.copyWith(id: _genId()) : agenda;
  ...
  void updateAgenda(String id, AgendaItem patch) {
    _agendaItems = _agendaItems.map((a) => a.id == id ? a.copyWith(
  ...
  void _scheduleReminderForAgenda(AgendaItem agenda) {
    if (agenda.status != AgendaStatus.pending) return;
    ...
    try {
      ...
      int repeatCount = 1;
      if (agenda.isMustDo || agenda.level == AgendaLevel.mustDo) {
        advanceMinutes = (_reminderRules['mustDo']?['advanceMinutes'] ?? 30) as int;
        repeatCount = (_reminderRules['mustDo']?['repeatCount'] ?? 5) as int;
      } else if (agenda.level == AgendaLevel.important) {
        advanceMinutes = (_reminderRules['important']?['advanceMinutes'] ?? 30) as int;
        repeatCount = (_reminderRules['important']?['repeatCount'] ?? 3) as int;
      } else {
        advanceMinutes = (_reminderRules['normal']?['advanceMinutes'] ?? 10) as int;
        repeatCount = (_reminderRules['normal']?['repeatCount'] ?? 1) as int;
      }
      ...
      _notification.scheduleAgendaReminder(
        ...
        isMustDo: agenda.isMustDo || agenda.level == AgendaLevel.mustDo,
      );
    } catch (e) {
      debugPrint('调度提醒失败: $e');
    }
  ...
  InventoryItem? findInventory(String name) {
    try {
      return _inventory.firstWhere((i) =>
        i.name == name || name.contains(i.name) || i.name.contains(name));
    } catch (_) {
      return null;
    }
  }

  void updateInventory(String name, double change, String unit, {String reason = '', String category = '其他'}) {
    final existingIndex = _inventory.indexWhere((i) =>
      i.name == name || name.contains(i.name) || i.name.contains(name));
  ...
  Map<String, dynamic> _parseVoiceText(String text) {
    ...
    // 多事程拆分：包含"还有""然后""再"且包含时间点
    if (hasCreateSignal && RegExp(r'还有|然后|再|和|，').hasMatch(t)) {
      final parts = t.split(RegExp(r'还有|然后|再|和|，')).where((s) => s.trim().isNotEmpty).toList();
      if (parts.length >= 2) {
  ...
  /// 综合统计
  StatsData computeStats(String range) {
    final ranking = computeBehaviorRanking(range);
    return StatsData(
      completionRate: computeCompletionRate(range),
  ...
  String _buildUserContext() {
    ...
    if (_inventory.isNotEmpty) {
      ...
      for (final inv in _inventory) {
        buffer.writeln('- ${inv.name}：剩余${inv.quantity}${inv.unit}');
      }
      buffer.writeln();
    }
...
- 数据中找不到的请直接说"我暂时没找到这个记录"''';
  ...
  String? _answerInventoryQuery(String q) {
    ...
    // 没找到特定物品，列出所有库存
    if (_inventory.isNotEmpty && (q.contains('库存') || q.contains('都有什么') || q.contains('都有多少'))) {
      final parts = <String>[];
      for (final item in _inventory) {
  ...
  String? _answerTimelineQuery(String q) {
    ...
    if (q.contains('今天') || q.contains('今日')) {
      ...
      final records = _timelineRecords.where((r) {
        final diff = DateTime.now().difference(r.time).inDays;
        return diff < days;
      }).toList();
  ...
  String get todayStr => MockData.todayStr;
